...
nCohort <- Farm_info[[3]]
nCows <- Farm_info[[4]]
nE0 <- floor(Farm_info[[7]]*nCows/nCohort) # Should maybe be updated
...
for (p in c(1:10)){
  ...
  for(k in 2:time){
    ...
    E_prop_sim[[k]] <- Farm %>% select(E_prop)
...
save(E_Prop_sim, file = "data/Simulations_SSL/E_prop_over_time.RData")
save(sim,file = "data/Simulations_SSL/Cohort_sim_c1.RData")

# Plots -------------------------------------------------------------------

# Transmission probability over time for simulation 1
for(g in 2:10){
po <-  E_Prop_sim[[g]]  
po <- bind_rows(po, .id = "Day")

po <- po %>%
  group_by(Day) %>%
  summarise(mean = mean(E_prop, na.rm = TRUE),
            sd = sd(E_prop, na.rm = TRUE),
            n = n()) %>%
  mutate(se = sd / sqrt(n),
         lower.prop = mean - qt(1 - (0.05 / 2), n - 1) * se,
